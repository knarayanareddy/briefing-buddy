...
async function completeSlackOAuth(code: string, state: string) {
  ...
  const res = await fetch(`${SUPABASE_URL}/functions/v1/slack-oauth-callback`, {
    method: "POST",
    headers,
  ...
  if (!res.ok) {
    ...
    throw new Error(err.error || `Slack OAuth failed (${res.status})`);
  }
...
export default function SlackOAuthCallback() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus("error");
      setErrorMessage(error === "access_denied" ? "You denied access to Slack." : error);
      return;
    }
      ...
      <div className="text-center space-y-6 max-w-md mx-auto p-8">
        {status === "processing" && (
          <>
            ...
            <h2 className="text-xl font-bold text-foreground">Connecting Slack...</h2>
        ...
        {status === "success" && (
          <>
            ...
            <h2 className="text-xl font-bold text-foreground">Slack Connected!</h2>
            <p className="text-sm text-muted-foreground">
              {teamName ? `Connected to workspace "${teamName}".` : "Your Slack workspace is now connected."}
              {" "}Configure channels to monitor in the Connectors page.
            </p>
